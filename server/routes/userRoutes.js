...
const registerUser = asyncHandler(async(req, res) => {
    ...
    if(user){
        ...
        res.status(400).send('We could not register user');
...
const passwordResetRequest = asyncHandler(async (req,res)  => {
    ...
    try {
        ...
        if(user){
            ...
            res.status(200).send(`We have send you a recovery email to ${email}`);
        }
    } catch (error) {
...
const passwordReset = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if(user){
            ...
            await user.save();
            res.json('your password has been upadted successfully.');
        } else {
            res.status(404).send('User not found.');
        }
    } catch (error) {
